    ...
    for export in [True, False]:
        for pat_id in range(55,70):
            ...
            if export and os.path.exists(os.path.join(status_path, 'Exported_Images_Disease_{}.txt'.format(pat_id))):
                continue
            elif not export and os.path.exists(os.path.join(status_path,
                                                            'Imported_Predictions_Disease_{}.txt'.format(pat_id))):
                continue
            try:
            ...
            for case in class_struct.patient.Cases:
                ...
                for exam in case.Examinations:
                    if export:
                        ...
                        fid = open(os.path.join(status_path,
                                                'Exported_Images_Disease_{}.txt'.format(pat_id)), 'w+')
                        fid.close()
                    else:
                        ...
                        fid = open(os.path.join(status_path,
                                                'Imported_Predictions_Disease_{}.txt'.format(pat_id)), 'w+')
                        fid.close()